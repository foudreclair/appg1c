...
<div class = "corps">
	...
	<button onclick='document.location.href="index.php?page=cmd" '>Voir toutes les commandes</button>
	<button onclick='document.location.href="index.php?page=cmd&voir=nonpaye" '>Voir les commandes non payées</button><br><br>
	<?php
...
while($don = $req->fetch_array(MYSQLI_ASSOC)){
	...
	<button onclick='document.location.href="Controleur/expcom.php?type=com&val=exp&id=<?php echo $don['Id'] ?>" '>Tout expédier</button>
	<?php
	if ($don['Payement']=='Non'){
	?>
	<button onclick='document.location.href="Controleur/valpay.php?val=pay&id=<?php echo $don['Id'] ?>" '>Valider le payement</button><br>
	<?php
	}
	else {
		?>
	<button onclick='document.location.href="Controleur/valpay.php?val=suppay&id=<?php echo $don['Id'] ?>" '>Annuler le payement</button><br>
	<?php
	}	
	?>
	<?php
	...
	<ul>
		...
		if ($achat['Expedition']=="Non"){
			?>
		<button onclick='document.location.href="Controleur/expcom.php?type=art&val=exp&id=<?php echo $achat['Id'] ?>" '>Expédier</button>
		<?php
		}
		else{
			?>
			<button onclick='document.location.href="Controleur/expcom.php?type=art&val=supexp&id=<?php echo $achat['Id'] ?>" '>Annuler l'expédition</button>
			<?php
		}
		?>
